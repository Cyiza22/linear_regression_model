...
try:
    ...
    raise RuntimeError("Model file 'linear_regression_model.pkl' not found. Make sure it's in the same directory.")

# Create FastAPI app
app = FastAPI()

# Add CORS middleware
origins = ["*"]  # Adjust this to specific origins for better security in production
app.add_middleware(
    CORSMiddleware,
    allow_origins=origins,
    allow_credentials=True,
...
class InputData(BaseModel):
    ...
    sleep_hours: float = Field(..., ge=0, le=24, description="Sleep hours per day (0-24)")
    social_hours: float = Field(..., ge=0, le=24, description="Socializing hours per day (0-24)")
    physical_activity_hours: float = Field(..., ge=0, le=24, description="Physical activity hours per day (0-24)")
